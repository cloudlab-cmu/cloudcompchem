...
water_input_dict = {"molecule": water_dict.copy(),
...
def water_expected_response():
    return {
        "converged": True,
        "energy": -76.471217696,
        "orbitals": [{'energy': -19.171027052211535, 'occupancy': 2.0},
        ...
        "distance_matrix": np.array([[0.        , 1.83048552, 1.83048552],
                                            [1.83048552, 0.        , 2.85977529],
                                            [1.83048552, 2.85977529, 0.        ]])
    }
...
def test_water():
    ...
    assert all(
        isclose(ex_orb["energy"], t_orb.energy) for ex_orb, t_orb in zip(expected_response["orbitals"], response.orbitals)
    )
